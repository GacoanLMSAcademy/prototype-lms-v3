...
const navItems = computed<MenuEntry[]>(() => {
  ...
  if (role === 'admin') {
    return [
      ...
      {
        label: 'Assessment Methods', icon: '📋',
        children: [
          { label: 'Multirater', to: '/admin/multirater', icon: '👥' },
          { label: 'Presentation', to: '/admin/presentation', icon: '🎤' },
          { label: 'Validation', to: '/admin/validation', icon: '✅' },
