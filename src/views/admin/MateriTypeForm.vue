...
const existing = isEdit ? materiTypes.find(t => t.id === route.params.id) : null

const name = ref(existing?.name ?? '')
const description = ref(existing?.description ?? '')

function save() {
  alert('Materi type saved (mock)')
  router.push('/admin/materi-type')
}
</script>

<template>
  <div>
    <h2 class="text-2xl font-bold mb-6">{{ isEdit ? 'Edit Materi Type' : 'New Materi Type' }}</h2>
    <div class="bg-white p-6 rounded shadow space-y-4 max-w-2xl">
      <div>
        <label class="block text-sm font-medium mb-1">Name</label>
        <input v-model="name" class="w-full border rounded px-3 py-2" placeholder="e.g. Komunikasi Efektif" />
      </div>
      <div>
