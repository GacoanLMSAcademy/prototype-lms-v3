...
import { skillTestMethods, formAssessments } from '@/data/mockData'
import type { MethodCategory } from '@/types'
const route = useRoute()
const router = useRouter()
const isEdit = !!(route.params.id && route.params.id !== 'new')
const existing = isEdit ? skillTestMethods.find(m => m.id === route.params.id) : null
...
const minAttempts = ref(existing?.minAttempts ?? 3)
function addCategory() { categories.value.push({ id: 'sc' + Date.now(), name: '', weight: 0, formAssessmentId: '' }) }
function removeCategory(idx: number) { categories.value.splice(idx, 1) }
function save() { alert('Skill Test saved (mock)'); router.push('/admin/skill-test') }
</script>
<template>
  <div>
    <h2 class="text-2xl font-bold mb-6">{{ isEdit ? 'Edit Skill Test' : 'New Skill Test' }}</h2>
    <div class="bg-white p-6 rounded shadow space-y-4 max-w-4xl">
      <div class="grid grid-cols-2 gap-4">
        ...
        <div><label class="block text-sm font-medium mb-1">Min Attempts</label><input v-model.number="minAttempts" type="number" min="1" class="w-full border rounded px-3 py-2"/></div>
      </div>
