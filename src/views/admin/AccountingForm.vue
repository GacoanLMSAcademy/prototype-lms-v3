...
import { accountingMethods, tests } from '@/data/mockData'
import type { AccountingCategory } from '@/types'
const route = useRoute()
const router = useRouter()
const isEdit = !!(route.params.id && route.params.id !== 'new')
const existing = isEdit ? accountingMethods.find(m => m.id === route.params.id) : null
const title = ref(existing?.title ?? '')
const description = ref(existing?.description ?? '')
const categories = ref<AccountingCategory[]>(existing?.categories ?? [])
function addCategory() { categories.value.push({ id: 'acc' + Date.now(), name: '', weight: 0, testId: '' }) }
function removeCategory(idx: number) { categories.value.splice(idx, 1) }
function save() { alert('Accounting saved (mock)'); router.push('/admin/accounting') }
</script>
<template>
  <div>
    <h2 class="text-2xl font-bold mb-6">{{ isEdit ? 'Edit Accounting' : 'New Accounting' }}</h2>
      ...
      <div>
        ...
        <div v-for="(cat, ci) in categories" :key="cat.id" class="border rounded p-4 mb-4 bg-gray-50">
          ...
          <div class="grid grid-cols-3 gap-3 mb-2">
            ...
            <div><label class="text-xs">Test (Essay)</label><select v-model="cat.testId" class="w-full border rounded px-2 py-1 text-sm"><option value="">-- Select --</option><option v-for="t in tests" :key="t.id" :value="t.id">{{ t.title }}</option></select></div>
          </div>
        </div>
      </div>
