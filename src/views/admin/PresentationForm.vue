...
import { presentationMethods, formAssessments } from '@/data/mockData'
import type { MethodCategory } from '@/types'
const route = useRoute()
const router = useRouter()
const isEdit = !!(route.params.id && route.params.id !== 'new')
const existing = isEdit ? presentationMethods.find(m => m.id === route.params.id) : null
...
function addCategory() { categories.value.push({ id: 'pc' + Date.now(), name: '', weight: 0, formAssessmentId: '' }) }
function removeCategory(idx: number) { categories.value.splice(idx, 1) }
function save() { alert('Presentation saved (mock)'); router.push('/admin/presentation') }
</script>
<template>
  <div>
    <h2 class="text-2xl font-bold mb-6">{{ isEdit ? 'Edit Presentation' : 'New Presentation' }}</h2>
