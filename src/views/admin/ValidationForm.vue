...
import { validationMethods, formAssessments } from '@/data/mockData'
import type { MethodCategory } from '@/types'
const route = useRoute()
const router = useRouter()
const isEdit = !!(route.params.id && route.params.id !== 'new')
const existing = isEdit ? validationMethods.find(m => m.id === route.params.id) : null
...
function addCategory() { categories.value.push({ id: 'vc' + Date.now(), name: '', weight: 0, formAssessmentId: '' }) }
function removeCategory(idx: number) { categories.value.splice(idx, 1) }
function save() { alert('Validation saved (mock)'); router.push('/admin/validation') }
</script>
<template>
  <div>
    <h2 class="text-2xl font-bold mb-6">{{ isEdit ? 'Edit Validation' : 'New Validation' }}</h2>
