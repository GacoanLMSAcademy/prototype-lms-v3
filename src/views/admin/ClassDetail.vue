...
const participantDetails = computed(
  () =>
    (cls?.participants.map((pid) => users.find((u) => u.id === pid)).filter(Boolean) as NonNullable<
      (typeof users)[number]
    >[]) ?? [],
)
const raterDetails = computed(
  () =>
    (cls?.raters.map((rid) => users.find((u) => u.id === rid)).filter(Boolean) as NonNullable<
      (typeof users)[number]
    >[]) ?? [],
)
const curriculumDetail = computed(() => curricula.find((c) => c.id === cls?.curriculumId))
const uploadFileItems = computed(
  () => curriculumDetail.value?.items.filter((i) => i.trainingMethodType === 'uploadFile') ?? [],
)
  ...
  <div v-if="cls">
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex gap-2">
        <button
          @click="router.push('/admin/classes/' + cls.id + '/progress')"
          class="text-sm bg-indigo-600 text-white px-3 py-1 rounded hover:bg-indigo-700"
        >
          📊 Progress
        </button>
        <button
          @click="router.push('/admin/classes/' + cls.id + '/edit')"
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
      <div class="bg-white p-4 rounded shadow">
        <p class="text-gray-500 text-sm">Program Type</p>
        <p class="font-medium">{{ programTypeDetail?.name ?? '-' }}</p>
      </div>
      <div class="bg-white p-4 rounded shadow">
        <p class="text-gray-500 text-sm">Category</p>
        <p class="font-medium">{{ programTypeDetail?.category ?? '-' }}</p>
      </div>
      <div class="bg-white p-4 rounded shadow">
        <p class="text-gray-500 text-sm">Curriculum</p>
        <p class="font-medium">{{ curriculumDetail?.title ?? 'Not assigned' }}</p>
      </div>
      <div class="bg-white p-4 rounded shadow">
        <p class="text-gray-500 text-sm">Instructor</p>
        <p class="font-medium">{{ users.find((u) => u.id === cls!.instructorId)?.name ?? '-' }}</p>
      </div>
      <div class="bg-white p-4 rounded shadow">
        <p class="text-gray-500 text-sm">Status</p>
        <p class="font-medium capitalize">{{ cls!.status }}</p>
      </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div class="bg-white rounded shadow p-4">
        ...
        <ul class="space-y-1">
          <li v-for="p in participantDetails" :key="p.id" class="text-sm">
            {{ p.name }} — {{ p.email }}
          </li>
          <li v-if="participantDetails.length === 0" class="text-gray-400 text-sm">None</li>
        </ul>
      </div>
      <div class="bg-white rounded shadow p-4">
        <h3 class="font-semibold mb-3">Raters ({{ raterDetails.length }})</h3>
        <ul class="space-y-1">
          <li v-for="r in raterDetails" :key="r.id" class="text-sm">
            {{ r.name }} — {{ r.email }}
          </li>
          <li v-if="raterDetails.length === 0" class="text-gray-400 text-sm">None</li>
        </ul>
      </div>
    </div>
    <div class="bg-white rounded shadow p-4 mt-6">
      ...
      <div
        v-for="(item, idx) in curriculumDetail?.items ?? []"
        :key="item.id"
        class="border-t py-2 flex items-center justify-between text-sm"
      >
        <span
          >{{ idx + 1 }}. {{ item.trainingMethodType }} ({{
            item.contentId.substring(0, 8)
          }}...)</span
        >
        <span class="text-xs bg-gray-200 px-2 py-0.5 rounded"
          >Weight: {{ item.weight }}% | Pass: {{ item.passingScore }}%</span
        >
      </div>
    ...
    <div class="bg-white rounded shadow p-4 mt-6">
      ...
      <div
        v-for="item in (curriculumDetail?.items ?? []).filter(
          (i) => i.trainingMethodType !== 'uploadFile',
        )"
        :key="item.id"
        class="border-t py-3"
      >
        ...
        <div v-if="item.trainingMethodType === 'inClass'" class="space-y-2">
          <div
            v-for="assignment in (cls.inClassInstructorAssignments ?? []).filter(
              (a) => a.trainingMethodId === item.id,
            )"
            :key="assignment.categoryId + assignment.materiId"
            class="text-sm bg-gray-50 rounded p-2 flex items-center justify-between"
          >
            <span>{{
              materis.find((m) => m.id === assignment.materiId)?.title ?? assignment.materiId
            }}</span>
            <span>{{ instructorName(assignment.instructorId) }}</span>
          </div>
          <p
            v-if="
              !(cls.inClassInstructorAssignments ?? []).some((a) => a.trainingMethodId === item.id)
            "
            class="text-sm text-gray-400"
          >
            No materi instructors assigned.
          </p>
        </div>
        <div
          v-else-if="
            ['multirater', 'presentation', 'validation', 'skillTest', 'verify'].includes(
              item.trainingMethodType,
            )
          "
          class="space-y-2"
        >
          <div
            v-for="assignment in (cls.assessmentAssessorAssignments ?? []).filter(
              (a) => a.trainingMethodId === item.id,
            )"
          ...
          >
            <span>{{
              users.find((u) => u.id === assignment.participantId)?.name ?? assignment.participantId
            }}</span>
            <span>{{ raterNames(assignment.raterIds) }}</span>
          </div>
          <p
            v-if="
              !(cls.assessmentAssessorAssignments ?? []).some((a) => a.trainingMethodId === item.id)
            "
            class="text-sm text-gray-400"
          >
            No participant assessors assigned.
          </p>
        </div>
        ...
        <div v-for="pid in cls.participants" :key="pid" class="text-sm bg-gray-50 rounded p-2 mb-2">
          ...
          <div v-for="item in uploadFileItems" :key="item.id" class="ml-2 mb-2">
            <p class="text-sm font-medium text-gray-700 mb-1">
              {{ item.contentId || 'Upload File' }}
            </p>
            <div
              v-for="uf in participantUploads(pid, item.id)"
              :key="uf.id"
              class="ml-2 text-xs text-gray-600 flex items-center justify-between"
            >
              <span>{{ uf.fileName }} ({{ uf.fileType }})</span>
              <a :href="uf.fileUrl" target="_blank" class="text-blue-600 hover:underline">Lihat</a>
            </div>
            <p
              v-if="participantUploads(pid, item.id).length === 0"
