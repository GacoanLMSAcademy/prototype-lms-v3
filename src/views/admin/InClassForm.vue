...
function addMateriToMateri(catIdx: number, materiId: string) {
  const cat = categories.value[catIdx]
  if (!cat) return
  if (!cat.materiIds.includes(materiId)) cat.materiIds.push(materiId)
}
function removeMateriFromMateri(catIdx: number, materiIdx: number) {
  const cat = categories.value[catIdx]
  if (!cat) return
  cat.materiIds.splice(materiIdx, 1)
}
function save() {
  alert('In-Class saved (mock)')
  router.push('/admin/inclass')
}
</script>
    ...
    <div class="bg-white p-6 rounded shadow space-y-4 max-w-4xl">
      ...
      <div>
        <div class="flex items-center justify-between mb-2">
          <h3 class="font-semibold">Materis</h3>
          <button
            @click="addMateri"
        ...
        <div v-for="(cat, ci) in categories" :key="cat.id" class="border rounded p-4 mb-4">
          <div class="flex items-center justify-between mb-2">
            <span class="text-sm font-medium">Materi {{ ci + 1 }}</span
            ><button @click="removeMateri(ci)" class="text-red-500 text-sm">Remove</button>
          </div>
          <div class="grid grid-cols-3 gap-3 mb-3">
            ...
            <div>
              <label class="text-xs">Materi Type</label
              ><select v-model="cat.materiTypeId" class="w-full border rounded px-2 py-1 text-sm">
                <option value="">--</option>
                <option v-for="mt in materiTypes" :key="mt.id" :value="mt.id">{{ mt.name }}</option>
              </select>
            </div>
          </div>
          <div class="grid grid-cols-2 gap-3 mb-3">
            ...
            <div>
              <label class="text-xs">Post-Test</label
              ><select v-model="cat.postTestId" class="w-full border rounded px-2 py-1 text-sm">
                <option value="">--</option>
                <option v-for="t in tests" :key="t.id" :value="t.id">{{ t.title }}</option>
              </select>
            </div>
          ...
          <div>
            ...
            <div v-for="(mi, mii) in cat.materiIds" :key="mi" class="flex items-center gap-2 mb-1">
              <span class="text-sm">{{ materis.find((m) => m.id === mi)?.title ?? mi }}</span
              ><button @click="removeMateriFromMateri(ci, mii)" class="text-red-400 text-xs">
                x
              </button>
            </div>
            <select
              @change="
                (e: any) => {
                  addMateriToMateri(ci, e.target.value)
                  e.target.value = ''
                }
              "
              class="w-full border rounded px-2 py-1 text-sm mt-1"
            >
              <option value="">+ Add Materi</option>
              <option
                v-for="m in materis.filter((mm) => !cat.materiIds.includes(mm.id))"
                :key="m.id"
                :value="m.id"
              >
                {{ m.title }}
