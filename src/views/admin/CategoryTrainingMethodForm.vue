...
const existing = isEdit ? trainingMethodTypes.find(t => t.id === route.params.id) : null

const name = ref(existing?.name ?? '')
const description = ref(existing?.description ?? '')

function save() {
  alert('Training method type saved (mock)')
  router.push('/admin/category-training-method')
}
</script>

<template>
  <div>
    <h2 class="text-2xl font-bold mb-6">{{ isEdit ? 'Edit Training Method Type' : 'New Training Method Type' }}</h2>
    <div class="bg-white p-6 rounded shadow space-y-4 max-w-2xl">
      <div>
        <label class="block text-sm font-medium mb-1">Name</label>
        <input v-model="name" class="w-full border rounded px-3 py-2" placeholder="e.g. Multirater, Presentation, Custom Method" />
      </div>
      <div>
