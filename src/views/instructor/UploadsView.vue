...
const myClassesWithUploadFile = computed(() =>
  classes.filter(cls => {
    if (cls.instructorId !== auth.userId && !(cls.inClassInstructorAssignments ?? []).some(a => a.instructorId === auth.userId)) return false
...
<template>
  <div>
    <h2 class="text-2xl font-bold mb-6">Upload File — Peserta</h2>

    <div v-if="myClassesWithUploadFile.length === 0" class="text-center py-12 text-gray-400">
      Tidak ada kelas dengan tugas upload file
    </div>

    <div v-for="cls in myClassesWithUploadFile" :key="cls.id" class="bg-white rounded shadow mb-4">
